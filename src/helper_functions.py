...
def plot_categorical_distribution(
    ...
    """Plot category frequencies ordered from most to least common."""
    plt.figure(figsize=(10, 5))
    data[column].value_counts().plot(kind="bar")
    plt.title(title or f"Distribution of {column} Categories")
    plt.xlabel(column)
    plt.ylabel("Count")
...
def numeric_vs_categorical_analysis(
    ...
    """Compare a numeric feature across categories and target groups."""
    display(
        data.groupby(categorical_column)[numeric_column]
        .describe()
        .round(2)
    )
    ...
    plt.title(
        f"{numeric_column} Distribution by "
        f"{categorical_column} and {target_column}"
    )
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()
...
def scale_numeric_features(
    ...
    train_scaled_values = pd.DataFrame(
        scaler.transform(train_inputs[numeric_cols]),
        columns=numeric_cols,
        index=train_inputs.index,
    )
    val_scaled_values = pd.DataFrame(
        scaler.transform(val_inputs[numeric_cols]),
        columns=numeric_cols,
        index=val_inputs.index,
    )
    train_scaled = pd.concat(
        [train_inputs.drop(columns=numeric_cols), train_scaled_values],
        axis=1,
    )[train_inputs.columns]
    val_scaled = pd.concat(
        [val_inputs.drop(columns=numeric_cols), val_scaled_values],
        axis=1,
    )[val_inputs.columns]
...
def preprocess_new_data(
    ...
    if scaler_numeric and numeric_cols:
        ...
        inputs = pd.concat(
            [inputs.drop(columns=numeric_cols), scaled_values],
            axis=1,
        )[inputs.columns]
